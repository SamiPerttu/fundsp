...
/// Multichannel single precision wave.
pub struct Wave32 {
    /// Vector of channels. Each channel is stored in its own vector.
    vec: Vec<Vec<f32>>,
    /// Sample rate of the wave.
...
impl Wave32 {
    ...
    /// Returns a mutable reference to the requested channel.
    pub fn channel_mut(&mut self, channel: usize) -> &mut Vec<f32> {
        &mut self.vec[channel]
    }
    ...
    /// Resets `node` and sets its sample rate.
    /// Does not discard pre-delay.
    pub fn render(sample_rate: f64, duration: f64, node: &mut dyn AudioUnit32) -> Self {
        assert!(node.inputs() == 0);
        assert!(node.outputs() > 0);
        node.reset(Some(sample_rate));
        let length = (duration * sample_rate).round() as usize;
        let mut wave = Wave32::with_capacity(node.outputs(), sample_rate, length);
        let mut i = 0;
        let mut buffer = Wave32::new(node.outputs(), sample_rate);
        let mut reusable_slice = Slice::<[f32]>::with_capacity(node.outputs());
        while i < length {
            let n = min(length - i, MAX_BUFFER_SIZE);
            buffer.resize(n);
            node.process(n, &[], reusable_slice.from_muts(&mut buffer.vec));
    ...
    /// Any pre-delay, as measured by signal latency, is discarded.
    /// Resets `node` and sets its sample rate.
    pub fn render_latency(sample_rate: f64, duration: f64, node: &mut dyn AudioUnit32) -> Self {
        assert!(node.inputs() == 0);
        assert!(node.outputs() > 0);
        let latency = node.latency().unwrap_or_default();
        // Round latency down to nearest sample.
        ...
        let duration = duration_samples as f64 / sample_rate;
        if latency_samples > 0 {
            let latency_wave = Wave32::render(sample_rate, duration + latency_duration, node);
            let mut wave = Wave32::with_capacity(node.outputs(), sample_rate, duration_samples);
            wave.resize(duration_samples);
            for channel in 0..wave.channels() {
                for i in 0..duration_samples {
                    wave.set(channel, i, latency_wave.at(channel, i + latency_samples));
            ...
            Wave32::render(sample_rate, duration, node)
        }
    }
    ...
    /// All zeros input is used for the rest of the wave if
    /// the duration is greater than the duration of this wave.
    pub fn filter(&self, duration: f64, node: &mut dyn AudioUnit32) -> Self {
        assert!(node.inputs() == self.channels());
        assert!(node.outputs() > 0);
        node.reset(Some(self.sample_rate()));
        let total_length = round(duration * self.sample_rate()) as usize;
        let input_length = min(total_length, self.length());
        let mut wave = Wave32::with_capacity(node.outputs(), self.sample_rate(), total_length);
        let mut i = 0;
        let mut input_buffer = Wave32::new(self.channels(), self.sample_rate());
        let mut reusable_input_slice = Slice::<[f32]>::with_capacity(self.channels());
        let mut output_buffer = Wave32::new(node.outputs(), self.sample_rate());
        let mut reusable_output_slice = Slice::<[f32]>::with_capacity(node.outputs());
        // Filter from this wave.
        while i < input_length {
            let n = min(input_length - i, MAX_BUFFER_SIZE);
            input_buffer.resize(n);
    ...
    /// All zeros input is used for the rest of the wave if
    /// the duration is greater than the duration of this wave.
    pub fn filter_latency(&self, duration: f64, node: &mut dyn AudioUnit32) -> Self {
        assert!(node.inputs() == self.channels());
        assert!(node.outputs() > 0);
        let latency = node.latency().unwrap_or_default();
        // Round latency down to nearest sample.
        ...
        if latency_samples > 0 {
            let latency_wave = self.filter(duration + latency_duration, node);
            let mut wave =
                Wave32::with_capacity(node.outputs(), self.sample_rate(), duration_samples);
            wave.resize(duration_samples);
            for channel in 0..wave.channels() {
                for i in 0..duration_samples {
                    wave.set(channel, i, latency_wave.at(channel, i + latency_samples));
