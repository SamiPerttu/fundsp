...
impl<T, F, E, R> Envelope2<T, F, E, R>
where
    T: Float,
    F: Float,
    E: Fn(F, F) -> R + Clone,
    ...
    R::Size: Size<T>,
{
    ...
    fn next_segment(&mut self, input: F) {
...
impl<T, F, E, R> AudioNode for Envelope2<T, F, E, R>
where
    T: Float,
    F: Float,
    E: Fn(F, F) -> R + Clone,
    ...
    R::Size: Size<T>,
{
    ...
    fn tick(
        ...
        if self.t >= self.t_1 {
            self.next_segment(convert(input[0]));
        }
        let output = self.value.clone();
        self.value += &self.value_d;
        self.t += self.sample_duration;
    ...
    fn process(
        ...
        if self.t >= self.t_1 {
            self.next_segment(convert(input[0][0]));
        }
        let mut i = 0;
        while i < size {
            let segment_samples_left =
            ...
            if loop_samples == segment_samples_left && i < size {
                self.next_segment(convert(input[0][i]));
            }
        }
    }
...
impl<T, F, E, R> Envelope3<T, F, E, R>
where
    T: Float,
    F: Float,
    E: Fn(F, F, F) -> R + Clone,
    ...
    R::Size: Size<T>,
{
    ...
    fn next_segment(&mut self, input0: F, input1: F) {
        if self.t_0 == F::zero() && self.t_1 == F::zero() {
            // Get the initial value.
            let value_0: Frame<_, _> = (self.envelope)(self.t_0, input0, input1).convert();
            self.value_0 = Frame::generate(|i| convert(value_0[i]));
        } else {
            self.t_0 = self.t_1;
            self.value_0 = self.value_1.clone();
        ...
        self.t_1 = self.t_0 + next_interval;
        let value_1: Frame<_, _> = (self.envelope)(self.t_1, input0, input1).convert();
        self.value_1 = Frame::generate(|i| convert(value_1[i]));
        self.t_hash = self
            .t_hash
            .wrapping_mul(6364136223846793005)
...
impl<T, F, E, R> AudioNode for Envelope3<T, F, E, R>
where
    T: Float,
    F: Float,
    E: Fn(F, F, F) -> R + Clone,
    ...
    R::Size: Size<T>,
{
    ...
    fn tick(
        ...
        if self.t >= self.t_1 {
            self.next_segment(convert(input[0]), convert(input[1]));
        }
        let output = self.value.clone();
        self.value += &self.value_d;
        self.t += self.sample_duration;
    ...
    fn process(
        ...
        if self.t >= self.t_1 {
            self.next_segment(convert(input[0][0]), convert(input[1][0]));
        }
        let mut i = 0;
        while i < size {
            let segment_samples_left =
            ...
            self.t += F::new(loop_samples as i64) * self.sample_duration;
            if loop_samples == segment_samples_left && i < size {
                self.next_segment(convert(input[0][i]), convert(input[1][i]));
            }
        }
    }
