...
impl<T: Float, F: Real> AudioNode for Follow<T, F> {
    ...
    fn tick(
        ...
        self.v1 = self.coeff * convert(input[0]) + rcoeff * self.v1;
        self.v2 = self.coeff * self.v1 + rcoeff * self.v2;
        self.v3 = self.coeff * self.v2 + rcoeff * self.v3;
...
impl<T: Float, F: Real, S: ScalarOrPair<Sample = F>> AudioNode for AFollow<T, F, S> {
    ...
    fn tick(
        ...
        self.v1 = self.time.filter_pole(v0, self.v1, self.acoeff, self.rcoeff);
        self.v2 = self
            .time
            .filter_pole(self.v1, self.v2, self.acoeff, self.rcoeff);
        self.v3 = self
            .time
            .filter_pole(self.v2, self.v3, self.acoeff, self.rcoeff);
        [convert(self.v3)].into()
    }
